...
builder.Services.AddCors(options =>
{
    options.AddPolicy("OnClass",
                      ...
                      {
                          builder
                            .WithOrigins(new string[] { @"http://localhost:3000", @"http://localhost:5135", @"https://localhost:7135" })
                            .AllowAnyHeader()
                            .AllowAnyMethod()
                            .AllowCredentials();
    });
...
builder.Services.AddDbContext<ApplicationContext>
    (options => options.UseMySql(ConnectionStringBuilder.Build(), Microsoft.EntityFrameworkCore.ServerVersion.Parse(@"8.0.27-mysql")), ServiceLifetime.Singleton);
